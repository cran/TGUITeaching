`ex_eda` <- function(eval=FALSE) {
	...
	plotScatterplotMatrix <- function() {
		...
		ifelse(Texists("aa"), aa <- as.numeric(evalq(tclvalue(aa), env = TGUIenv)), aa <- 0)
		ifelse(Texists("bb"), bb <- as.numeric(evalq(tclvalue(bb), env = TGUIenv)), bb <- 1)
		ifelse(Texists("cc"), cc <- as.numeric(evalq(tclvalue(cc), env = TGUIenv)), cc <- 1)
		ifelse(Texists("dd"), dd <- as.numeric(evalq(tclvalue(dd), env = TGUIenv)), dd <- 0)
		
		ifelse(Texists("diagonal"),	diagonal <- evalq(tclvalue(diagonal), env = TGUIenv), diagonal <- 1)		
		diagLabels <- c("histogram", "density", "boxplot", "qqplot")	
		ergDiag <- diagLabels[as.integer(diagonal)]			
		
		ifelse(Texists("smoothing"), smoothing <- evalq(tclvalue(smoothing), env = TGUIenv), smoothing <- FALSE)	
